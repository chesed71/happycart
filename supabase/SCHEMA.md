...
> 롤백 대조용) 상태다. 0018 현재까지 drop 되지 않았고 dev·prod 양쪽에 남아 있다
> (0016 은 drop 이 아니라 `upload_promoted_product` RPC 였다). 신규 적재는
> masters/barcodes 로 하며, drop 은 필요할 때 별도 마이그레이션에서 한다.
...
컬럼·제약은 마이그레이션 이력 참조. **0018 현재까지 drop 되지 않았다** — dev·prod 모두
테이블이 남아 있으며(롤백 대조용), 앱은 이 테이블을 읽지 않는다. drop 은 별도
마이그레이션에서 한다.
